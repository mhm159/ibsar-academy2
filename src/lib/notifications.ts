/**
 * Ibsar Academy — Advanced Notification System
 *
...
const TEMPLATES: Record<NotificationType, (params: Record<string, string>) => NotificationTemplate> = {
  ...
  TEACHER_APPROVED: (p) => ({
    ...
    body: 'مرحباً بك في أكاديمية إبصار! تم اعتماد حسابك كمعلم.',
    link: '/teacher',
  }),
  TEACHER_REJECTED: (p) => ({
  ...
  WELCOME: (p) => ({
    title: '🎉 مرحباً بك في أكاديمية إبصار!',
...
export async function sendNotification(
  ...
  if (user.phone && (whatsappEnabled || options.forceWhatsApp) && !inQuietHours) {
    const waMessage = `🎓 *أكاديمية إبصار*
